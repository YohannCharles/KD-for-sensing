...
- `src/kd_sensing/engine/objective_metadata.py` 已删除；objective 名称、默认 metric、metric alias 和 mode 表由 `src/kd_sensing/engine/objectives/registry.py` 维护，history fields 与 TensorBoard scalar schema 在 `src/kd_sensing/engine/objectives/history.py`，runtime metadata/validation helper 在 `src/kd_sensing/engine/objectives/metadata.py`。内部代码和测试不得再从旧 facade 导入。
...
- `prune-remaining-overengineered-surface` deleted: `src/kd_sensing/_typing.py`、`src/kd_sensing/engine/objective_metadata.py`、`src/kd_sensing/data/dataset_runtime.py`、`src/kd_sensing/config/canonical_recipes/common.py`、`src/kd_sensing/baselines/beambench/image_ae_gps.py`、`scripts/analyze_csi_hardening_sweep.py` 和 `tests/test_csi_hardening_sweep_analysis.py`。
- `prune-remaining-overengineered-surface` merged: `registry_self_check` 覆盖迁回 `tests/test_component_registry.py`，canonical recipe deep merge 统一使用 `kd_sensing.config.io.deep_merge`，`migration_guards.py` 复用单一 config-relative path helper 并删除低价值 retired-route 深递归 value scan，空 package marker 不再维护空 `__all__`，JEPA benchmark owner 显式导入 common 符号而不再 `import *`。
- retained-with-reason: `src/kd_sensing/data/dataset_descriptors.py` 仍保留两个轻量 dataclass，因为 `profile_for()`、`to_dict()` 和 validation 错误信息提供实际行为；若 descriptor 扩展仍只是静态表，再改成 plain dict。
