...
- `scripts/run_btapa_experiments.sh` 属于 shell_orchestration/local manual。职责是本地串行或有界并行运行 `configs/scene31/*btapa*.yaml`；输出限定为 ignored `logs/` 和 `outputs/scene31/`，dry-run 只能打印当前 `kd-sensing-train` 命令。
