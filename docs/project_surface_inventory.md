...
`refactor-project-governance-and-scene31-surface` 收口后重新扫描并补入主线演进台账：这是当前工作树 on-disk 只读扫描口径，包含尚未提交但已落盘的拆分 helper；tracked-only 口径可用 `git ls-files` 复核，可能略低。扫描范围为 `src/kd_sensing/`、`tests/`、`scripts/`、`configs/`、仓库根 Markdown 和 `docs/` Markdown：当前 `src/kd_sensing` 约有 264 个 Python 文件，`tests/` 约有 70 个 Python 文件，`scripts/` 有 43 个 Python/shell 文件，`configs/` 有 226 个 YAML；仓库根目录有 8 个 Markdown，`docs/` 有 17 个 Markdown。本基线排除 `dataset/`、`outputs/`、`logs/`、cache、checkpoint、下载压缩包、`.pytest_cache/`、`__pycache__/` 和其它本地运行产物；这些数字只作为趋势信号和右尺寸化上下文，而非硬 KPI。
