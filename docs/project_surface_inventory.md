...
- shell_orchestration: `scripts/run_csi_hardening_matrix.sh`、`scripts/run_m2beam_single_modal_scene31_queue.sh`、`scripts/run_next_v3_experiments.sh`、`scripts/run_rbma_strong_encoder_4gpu_queue.sh`。这些 shell 只编排本地 GPU/日志策略，输出限定在 ignored `logs/`、`outputs/scene31/` 或 CSI hardening output root；不得升级为 package CLI。DeepSense GPS circular soft-label、MMW GPS circular soft-label ablation 和 MMW sunny modal15 shell wrappers 已退役，历史本地输出只通过 runtime cleanup/organize manifest 审计。
- local/manual artifact: `scripts/run_rbma_missing_workflow.py` 是本地 bounded-concurrency runner，默认只启动 `kd-sensing-train` 运行 RBMA/weighted-sum missing-modality experiment configs，输出限定为 ignored `outputs/scene31/` 和 `logs/`；删除/收敛条件是包内 manifest runner 覆盖同等配置列表、auto-resume、worker override 和 dry-run 输出。
...
- `scripts/run_m2beam_single_modal_scene31_queue.sh` 属于 shell_orchestration。职责是用固定四 GPU 映射运行 Scene31 单模态强 encoder 配置；输出限定为 ignored `logs/m2beam_single_modal_scene31/` 和 `outputs/scene31/`，删除触发条件是对应 checkpoint 生成流程迁入 package runner 或不再需要强 encoder ablation。
...
- `scripts/run_rbma_strong_encoder_4gpu_queue.sh` 属于 shell_orchestration。职责是用固定 GPU 映射运行 strong-encoder RBMA/weighted-sum ablation 并调用 summary helper；输出限定为 ignored `logs/rbma_strong_encoder_4gpu/` 和 `outputs/scene31/`。
