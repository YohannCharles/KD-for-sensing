...
4. 读 active change 的 `proposal.md`、`design.md`、`tasks.md` 和 `specs/**/*.md`；没有 active change 时，优先读 `openspec/specs/` 中当前 specs。
5. 读 README 和 `docs/` 中对应 workflow 文档，确认当前推荐入口、退役说明和验证建议。
6. 最后看源码、测试和 `git status --short`，确认实际实现、未提交改动和 ignored runtime artifacts 没有被误当作源码需求。
...
4. 当前 `openspec/specs/` 中已归档为当前需求的 specs。
5. README、`docs/` workflow、`docs/project_surface_inventory.md` 和复现说明。
6. 源码和测试中已经存在的实现契约。
7. `openspec/changes/archive/`、历史报告、旧研究笔记、本地数据和运行产物。

OpenSpec archive、历史报告和本地产物不能覆盖当前 specs。当前打开文件也不等于项目权威入口，尤其不要从 generated metadata 或输出目录反推当前支持面。
...
| OpenSpec artifact | active change 的 proposal/design/spec/tasks、当前 specs、`openspec status` | `openspec/changes/<change>/` 或 `openspec/specs/` | `openspec validate <change> --strict`，必要时 `openspec status --change <change>` |
| 文档生命周期 | AGENTS 文档边界、README 文档索引、inventory 文档生命周期分类 | README、`AGENTS.md`、`docs/*.md`、OpenSpec 文档 | 架构边界测试；检查不把历史或退役路线写成当前推荐入口 |
...
- OpenSpec archive：`openspec/changes/archive/` 是历史记录，只能解释演进过程；当前需求以 active change 和 `openspec/specs/` 为准。
