...
- MMW pooled metadata 提供 `condition` 与 `scenario`，可形成 sunny/rainy/foggy 和 15 个 domain。PCPF 模型 forward 禁止读取 weather、domain、当前/未来 CSI、未来 channel、path、beam power或历史 beam；唯一 CSI 输入是显式启用的样本自身五帧历史 sparse CSI。
- 历史 CSI/TSPC runner、模型与配置不再保留；PCPF 只保留固定 simulator、codebook、cache、encoder 与 dataset sidecar。
...
| Stage 3 | 每模态 temperature、tau、显式启用时 eta/风险系数 | 只接受 Stage 2 validation-best；必须绑定未截断且通过的 gate JSON、SHA256 和相同 expert fingerprint |
