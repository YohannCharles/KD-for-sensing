...
| 2026-06 | JEPA visual analysis 与 GPS shortcut benchmark | `kd-sensing-jepa-visual-analysis`、`kd-sensing-jepa-gps-shortcut-benchmark`、`CLAIM-JEPA-VIS-DIAG`、`CLAIM-JEPA-SHORTCUT-PENDING` | 仅看 clean accuracy 不足以证明 JEPA 学到视觉语义，需要 shortcut、attention、embedding 和 stress evidence。 | visual analysis 是 diagnostic-only；shortcut smoke 只验证 schema；BeamBench-fair benchmark pending；真实 manifest 缺 checkpoint 会标 `unavailable`，strict 字段缺失或不一致会标 `not_comparable`。 | 作为 claim gate，不单独当性能结论。 | 创新点从“准确率提升”扩展为“模型在 GPS shortcut 和图像退化下是否仍使用视觉证据”。 | 用真实 checkpoint 替换 smoke manifest，并补 weights/config/split/sample_count/label_space/metric_profile/difficulty_digest/normalization_artifact/checkpoint_provenance/seed 后生成 robustness/drop/shortcut reliance 表。 |
| 2026-06 | Predictive JEPA robustness | `image_gps_jepa_predictive_hybrid_beambench_fair_lowmem.yaml`、`jepa_gps_shortcut_benchmark_predictive_robustness_smoke.yaml` | 当前图像不可观测或 GPS 受扰动时，history 和 predictive representation 是否能稳住 beam prediction。 | 训练 profile pending；完整 clean + image/GPS stress-curve real benchmark 尚未登记；缺真实 checkpoint 或 strict comparability 字段时只会产出 `unavailable` / `not_comparable` gate。 | 保留为独立鲁棒性主线，不等同 GPS-query pooling。 | 论文问题可写为“面向感知不可观测性的预测式 JEPA beam robustness”。 | 先训练 audited Image ResNet+GPS、JEPA baseline 和 predictive hybrid，再跑带 weights/config/split/sample_count/label_space/metric_profile/difficulty_digest/normalization_artifact/checkpoint_provenance/seed 的 real manifest。 |
...
| 2026-07-01 至 2026-07-03 | U-MaskBeamJEPA / RBMA / Scene31 next-round | `configs/fusion/experiments/rbma_missing_workflow/*`、`configs/scene31/next_round/experiment_manifest.*`、`add-scene31-adaptive-sampler-beamsoft-loss` | 旧 `proto_sampler_uniform_es40` 先作为 missing-modality 主胜者进入 next-round，验证 adaptive pattern exposure 与 beam topology soft supervision。 | RBMA claim pending；Scene31 BC change artifacts/tasks complete；真实训练输出仍应留在 ignored `outputs/scene31_next_round/`。 | weighted_sum / AMBER-style mask 作为当时下一轮主线，RBMA、condBTAPA、weakKD 等只作对照或局部候选。 | 两个可写机制：按 pattern 困难度自适应采样，以及利用 circular beam 邻域结构做 supervised soft target。 | 后续 summary 已改以 `proto_randomdrop_subset_es40` 为 reference；uniform 结果保留为 ablation。 |
| 2026-07-04 | Scene31 subset reference / maskfix / reliability follow-up | `configs/scene31/baseline_pack/*`、`configs/scene31/subset_reliability/*`、`fix-scene31-subset-reference-mask-eval` | baseline pack 显示 `proto_randomdrop_subset_es40` 更适合作为当前缺失模态 proto reference；同时 AMR/AMBER-lite fresh eval 暴露 missing mask 传递需要显式诊断。 | 已补 modular missing-mask forward 契约、mask_suspect 标记、subset reference summary、reliability fusion weight logging 和 runner；真实 reliability/PatternFiLM d8 长训结果仍 pending。 | `proto_randomdrop_subset_es40` 是新 reference；`proto_sampler_uniform_es40` 降级为 ablation；AMR/AMBER-lite 旧 run 只允许加载 best checkpoint 重评。 | 可写机制收窄为 randomdrop subset exposure、mask-weighted reliability fusion 和 randomdrop subset + PatternFiLM d8。 | 先跑 maskfix fresh eval 与 combined summary，再按三 seed 非 suspect 结果决定 reliability/PatternFiLM 是否 promote。 |
| 2026-07-05 | Scene31-34 main missing-modality workflow | `stabilize-main-experiment-evidence`、`scripts/run_scenes31_34_main.sh`、`outputs/scenes31_34_main_lmdb/summary` | 单场景 Scene31 与 Scene31-34 quick seed1 不足以支撑论文主结论，需要 pooled multi-scene seed1-5、per-scene 稳定性、missing_count=0/1/2/3 退化曲线、classifier baseline、external-lite baseline 和 compute profile。 | core proto n=5 本地 summary 可见 `proto_randomdrop_subset_es40` 仍为 Avg-Missing winner；summary 生成 `final_evidence_checklist.csv/md` 并把 classifier、external-lite、fresh eval、missing-count、per-scene、compute、paper table 和 conclusion 的 `pending` / `incomplete` caveat 带入 paper tables/conclusion；真实产物继续留在 ignored `outputs/`。 | Scene31-34 升级为缺失模态主实验；主方法候选冻结为 prototype + random subset exposure；Uniform 是 ablation；reliability fusion 和 PatternFiLM 不晋升；mask_suspect=true external rows 不进 ranking。 | 论文贡献聚焦 random subset exposure 在缺失比例增加时退化更慢，prototype head 相比 ordinary classifier 的增益，以及训练策略无额外推理成本，而不是继续搜索新模块。 | 用 `classifier_seed123` 补普通 classifier，用 `external_lite_seed1` 补 AMR/AMBER-lite maskfix seed1，用 `summarize_final_all` 重新生成 checklist、profile、paper tables 和 final conclusion。 |
...
| Predictive robustness | predictive hybrid + stress benchmark | 历史帧和预测表征能否缓解图像缺失、图像噪声和 GPS 噪声。 | clean anchor、stress curves、`margin_vs_resnet_dba`、weights/config/split/sample_count/label_space/metric_profile/difficulty_digest/normalization_artifact/checkpoint_provenance/seed。 |
| GPS shortcut 诊断 | shortcut benchmark / visual analysis | 模型提升是否来自视觉语义，而不是 GPS shortcut。 | 真实 checkpoint matrix、shortcut reliance、CxD crossing、failure decomposition、weights/config/split/sample_count/label_space/metric_profile/difficulty_digest/normalization_artifact/checkpoint_provenance/seed。 |
