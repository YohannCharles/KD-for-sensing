...
fusion configs may use their own encoder depth or student branch. Canonical fusion virtual configs use
`teacher_no_kd` and `student_no_kd`; `experiment.name` and `output.run_name`
...
Each slug has loadable canonical virtual paths `<slug>_teacher_no_kd.yaml` and
...
input fields from the modality contract. Generated fusion student configs use
...
New fusion extensions should default to the no-KD mainline; a legacy KD fusion baseline requires a separate proposal or an explicit tracked entity config with legacy lineage metadata.
