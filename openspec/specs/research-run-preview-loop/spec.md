# research-run-preview-loop Specification

## Purpose
定义当前研究 workflow 的安全预览闭环，包括无训练 happy path、静态 evidence QA、实验预算 manifest、run recipe 和环境 fallback；默认不读取真实 `dataset/`、不加载 checkpoint、不启动长训练，也不把 ignored runtime artifacts 纳入源码。
## Requirements
