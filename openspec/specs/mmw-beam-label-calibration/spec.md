# mmw-beam-label-calibration Specification

## Purpose
定义 MMW beam label calibration 的显式 opt-in 配置、raw 与 calibrated label space 映射、分布重排、provenance metadata、fitting 边界和诊断输出，确保启用校准时所有 class-indexed label artifact 可追溯且不污染未启用的默认 raw label 工作流。
## Requirements
