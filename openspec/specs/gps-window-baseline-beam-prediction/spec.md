# gps-window-baseline-beam-prediction Specification

## Purpose
定义仅基于历史 GPS/pose 滑动窗口的非神经网络 beam prediction baseline，约束其输入防泄漏边界、几何滤波输出、全场景评估产物和参数搜索记录，使该 baseline 可复现并可与 HiST-Beam 主线分开审计。
## Requirements
...
#### Scenario: target support holdout selection
- **WHEN** 用户在 `calibration_mode=target_adapt` 下配置 support holdout
- **THEN** 系统 MUST 将 target_adapt labeled support 确定性拆分为 calibration-fit 与 selection 子集
- **AND** direction、offset、boresight 或 fallback 统计 MUST 只从 calibration-fit 子集估计
- **AND** sweep 排名 MUST 使用 selection 子集指标
- **AND** metadata MUST 记录 fit/selection split 名称和样本数

#### Scenario: boresight angle calibration
- **WHEN** 用户配置 `boresight_angle_degrees` 或启用 `auto_calibrate_boresight_angle`
- **THEN** baseline MUST 在 angle-to-beam 映射前应用 `calibrated_azimuth = relative_azimuth - boresight_angle_degrees`
- **AND** 自动 boresight 估计 MUST 只使用合法 calibration split 的标签
- **AND** 输出 metrics 和 prediction diagnostics MUST 记录 effective boresight angle 与 boresight score
