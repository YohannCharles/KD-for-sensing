...
## Purpose
定义 MMW Town GPS Top8 candidate manifest 的 supporting-only 生成、校验和诊断契约，确保当前 BGAM workflow 可从 GPS v2 frozen logits 重新计算 mapped 64-beam candidate 集合，并和 scene-specific label-space、support/query metadata、normalized gain 诊断保持一致；standalone Top8 CLI/config 已退役。
## Requirements
