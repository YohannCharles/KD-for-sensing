# html-evidence-dashboard Specification

## Purpose
定义 research dashboard 的静态 HTML evidence dashboard 能力，覆盖离线渲染、candidate-only/readiness 展示、输出产物边界、HTML escaping 和 focused validation。该能力只生成 ignored output root 或用户显式路径下的证据视图，不替代正式 claim registry、实验协议或 OpenSpec current specs。
## Requirements
