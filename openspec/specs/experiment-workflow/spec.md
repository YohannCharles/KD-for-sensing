...
结构重构后，默认 image-only 和 image+radar 工作流 MUST 通过新脚本保持当前算法的核心行为语义，包括默认序列长度、预测步数、类别数、KD 模式、teacher 权重选择、early stopping、gradient clipping、checkpoint 恢复和指标计算。
