...
保持 current 文档与 MMW T2/baseline 和受限 DeepSense6G T2 支持面一致，防止已删除路线通过 README、维护索引、claim 文档或 current spec 重新被描述为可用能力。

## Requirements

### Requirement: current 文档只描述 retained surface

README、导航、维护索引、实验协议、claim registry 与 current specs MUST 只把 MMW T2、S1、AMBER-Full、RMBP-MM 及其 MMW runtime，以及 DeepSense6G Scene31–34 的受限 T2 四模态主线描述为 current。
