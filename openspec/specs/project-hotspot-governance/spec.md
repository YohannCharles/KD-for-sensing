...
#### Scenario: streamline waves 完成后防止热点回流
- **WHEN** 后续 change 修改 `MMWDataset`、`trainer._train_inner`、`run_evaluation_pass`、`ModularSequenceModel.forward` 或 `run_mmw_town_gps_v2`
- **THEN** 变更 MUST 保持 adapter/context/stage helper/artifact writer 的职责边界
- **AND** 若单个入口重新吸收已迁出的 family setup、resource build、batch step、diagnostics assembly 或 protocol artifact writing，inventory 或架构测试 MUST 要求拆回窄 owner 或登记新的 split-next 理由
