...
### Requirement: Scenario 23 descriptor for AMR-Net_gps_image
DeepSense6G 场景选择 MUST 支持 AMR-Net_gps_image 复现所需的 Scenario 23 描述符。描述符 MUST 包含 `scene_id: 23`、`scene_slug: scene23`、别名、默认数据根目录、legacy 数据根目录、默认 train/test CSV 名和 scene-scoped 输出目录规则。
...
- **AND** source audit 或 runtime metadata MUST 记录这些 CSV 名来自 AMR-Net_gps_image 复现协议
...
#### Scenario: 单场景输出隔离
- **WHEN** AMR-Net_gps_image runner 在 Scenario 23 上训练或评估
- **THEN** 默认 run output MUST 位于 `outputs/scene23/<run_name>/` 或 `outputs/analysis/amr_net_gps_image/<run_id>/`
