...
## Purpose
定义 DeepSense6G GPS+LiDAR BGAM reranker 的数据 manifest、GPS geometry prior、LiDAR spatial encoding、候选重排训练和评估产物契约，确保该 workflow 作为显式 opt-in 的当前主线能力运行，并与 frozen GPS v2 prior、TopK candidate selector 和 64-beam circular label 语义保持可审计一致。
## Requirements
