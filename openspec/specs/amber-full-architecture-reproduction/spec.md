...
系统 MUST 提供 paper-aligned AMBER full architecture core，用于融合 image、LiDAR、radar、GPS 和历史 beam index 表征。该 core MUST 支持输入 embedding、time/modality positional embedding、learnable fusion token、modality-specific transformer、modality-fusion transformer 和 beam prediction head 所需输出。
...
- **THEN** 系统 MUST 构建 image、LiDAR、radar、GPS 和历史 beam embedding 的输入路径
- **AND** representation core MUST 接收统一的 `[B,K,T,D]` 多模态特征或等价 token 表征
...
系统 MUST 为 AMBER full reproduction 提供 CMA/Class-Former 对齐模块或等价可测试组件。该组件 MUST 使用 class queries 和可用模态/fusion 表征计算 contrastive training payload，并 MUST 支持配置化 temperature、embedding dimension 和 loss weight。

#### Scenario: CMA payload 可计算 contrastive loss
- **WHEN** AMBER full 配置启用 CMA contrastive loss
- **THEN** 模型 forward MUST 输出 contrastive loss 所需的 query embeddings、positive/negative modality embeddings 或等价 logits
