...
项目 MUST 只声明 T2/baseline 必需的 public console scripts：`kd-sensing-train`、`kd-sensing-evaluate` 和 `kd-sensing-preprocess`。T2/baseline 的多 seed orchestration、固定 mask evaluation 和 summary MUST 作为 inventory 中有 owner 的 local/manual scripts 保留；runs、runtime cleanup、paper export、U-Mask matrix、MMW GPS v2、physics inspect 及所有历史 CLI MUST 从 public surface 删除。
...
- **THEN** inventory MUST 记录其 owner、输出边界和删除条件
