...
- **BREAKING** 将 `scenario9.py` 承载的 DeepSense6G dataset 主实现迁移到场景中立模块，并删除 `Scenario9Dataset`、`Scenario31Dataset`、`Scenario32Dataset` 兼容类和 `data.dataset.type: scenario9|scenario31|scenario32` 旧入口。
- **BREAKING** 删除 `kd_sensing.engine.builders`、`kd_sensing.data.transforms`、`kd_sensing.data.transform_ops._legacy` 等已由窄模块替代的兼容 facade；内部代码、测试和文档统一改用职责明确的模块路径。
...
- 影响测试：需要删除或重写所有直接导入 `Scenario9Dataset`、`kd_sensing.engine.builders`、`kd_sensing.data.transforms`、legacy fusion config alias、旧类名 alias 和 legacy weight fallback 的测试；新增旧入口拒绝与无残留引用检查。
