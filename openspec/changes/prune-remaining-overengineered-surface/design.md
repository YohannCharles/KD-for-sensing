...
- `dataset_descriptors.py` 是否完全删除，还是保留一个薄查询函数并改用 `modalities.py` 数据源，需实现时按调用方最小 diff 决定。
- `runtime_artifact_cleanup.py` 中哪些 legacy archive 分类仍是安全删除必要信息，需要以 tests 为准，不凭审美删除。
- `models/fusion/__init__.py` 是否保留最小 public imports，取决于 README/docs 是否仍把 package import 当公开 API；内部测试必须迁到 owner module。
