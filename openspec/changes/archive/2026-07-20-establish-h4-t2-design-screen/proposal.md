...
- 新增显式 `umask_h4_v1` training profile 与 `umask_router_nopattern_v1` 开发主线 router architecture profile：前者固定 AdamW、`lr=5e-4`、`weight_decay=3e-4`、40-epoch cosine warm restart，后者只关闭 router pattern feature；新的 T2/S1 主线 launcher、resolved config、checkpoint 和汇总共同记录二者指纹。
...
- 增加 profile/候选 fingerprint、训练完成性和 identity/mask 一致性校验；RouterNoPattern 在固定 inner-mask 与多 seed/outer evidence 前永久标记为 development-only，不更新论文 claim 或替换进行中的 BPA/CMA 正式消融。
...
- `training-evaluation-runtime`: 要求 MMW 训练和评估记录并校验 training/router profile、结构候选和 recipe 指纹，禁止不同 profile 的结果混合。
