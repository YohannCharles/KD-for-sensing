...
固定 Joint20/40/60/80、15 域和 10,000 次 paired-domain bootstrap 的筛选结果为 `8/8` 候选未通过预注册 Gate。所有候选相对 train-fit static prior 的方向均为正，但 Joint40/60/80 合并材料性阈值、部分置信区间或 corrupted-cell 降权率未同时满足；同时所有候选的 normalized gain 均未稳定超过 frozen Current Router。因此本 change 不晋级任何候选，不补 seed2--5，也不运行 pure-Drop 晋级检查。

训练诊断显示 `fused_utility` 已接近 0.99，而 quality/monotonic loss 在校准后期反而上升，表明连续 expected-utility 与最终 argmax 通信指标存在饱和/竞争。任何后续决策对齐目标必须作为独立 change 预注册，不得在本 change 内事后修改 Gate 或调小阈值。
