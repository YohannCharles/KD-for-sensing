...
- 新增 claim-ineligible 的 `deepsense6g_router_falsification_v1`：仅复用当前中断在 epoch20 的 T2 seed1 pooled checkpoint，在 Scene31--34 同一测试身份上比较 Learned、Uniform、全局 Clean 静态先验和通信效用 Oracle；按白天/夜间与固定 whole/token 缺失分解，用于决定是否值得恢复正式 40 epoch Router 证据，不自动修改模型或主张。
