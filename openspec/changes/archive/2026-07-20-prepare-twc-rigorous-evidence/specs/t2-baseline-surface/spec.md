...
### Requirement: DeepSense6G Router falsification 必须隔离动态融合与静态模态先验
系统 MAY 在正式 DeepSense6G 次级证据完成前，以 `deepsense6g_router_falsification_v1` 复用一个未完成训练的 T2 pooled checkpoint进行开发诊断。Learned、availability-normalized Uniform、全局 Clean 静态先验与逐样本通信效用 Oracle MUST 使用相同的 unimodal logits、availability、target、future beam power、sample identity 和 checkpoint；产物 MUST 标记 checkpoint epoch、训练未完成、Scene31--34 pooled parent identity 和 `claim_eligible=false`。

#### Scenario: 按真实 day/night 场景反证动态 Router
- **WHEN** evaluator 在 Scene31--34 pooled test 上运行 Router falsification
- **THEN** Scene31--34 MUST 只作为同一 checkpoint 的测试分片，不能成为独立训练任务
- **AND** fixed mask MUST 覆盖 Clean、全部 Drop1/Drop2/Drop3 与集合级平衡 Token20/40/60/80/90
- **AND** summary MUST 同时报告 pooled、逐 scene、day/night 的 Learned-Uniform、Learned-GlobalCleanPrior 与 Oracle gap
- **AND** 若 Learned 未有意义地优于 GlobalCleanPrior，系统 MUST 保留负结果并不得以更难数据集为由升级动态可靠性 claim
