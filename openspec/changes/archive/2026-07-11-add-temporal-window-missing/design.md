...
- 提供 H5/P1 local/manual launcher、evaluation、summary 和 focused tests。
...
8. AMBER Full 和 RMBP-MM 的 `[B,5,4]` mask 接入先走配置与 batch metadata。
...
S1-S4 或其它 temporal-router 拓扑不属于本 change 的延期工作。只有 H5/P1 固定 mask matrix 产生可比较证据，且结果表明现有 temporal mean、`supervised_router`、`pcpg`/`bprr` controls 无法回答明确研究问题时，才新建独立 OpenSpec change。新 change 必须重新定义最小 router 拓扑、oracle/soft-target 语义、配置拒绝边界、focused tests 和 claim gate；不得复用本 change 中已删除的 wrapper suite 作为事实前提。
