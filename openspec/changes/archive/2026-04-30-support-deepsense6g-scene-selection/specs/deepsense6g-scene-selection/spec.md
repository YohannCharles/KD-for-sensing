...
项目 MUST 支持通过配置选择 DeepSense6G 场景。`data.dataset.scene` MUST 接受整数和字符串别名，首批 MUST 支持 Scenario 9 与 Scenario 32。未显式设置场景时，通用 DeepSense6G 配置 MUST 默认使用 Scenario 32；旧 `data.dataset.type: scenario9` 配置 MUST 继续解析为 Scenario 9。
...
- **WHEN** 用户设置 `data.dataset.scene: 9`
...
#### Scenario: 使用场景默认路径
- **WHEN** 用户选择 Scenario 32 且未显式设置 `data.dataset.data_root`
- **THEN** 系统 MUST 使用 Scenario 32 的默认数据根目录
- **AND** train/test dataset MUST 使用该场景的默认 split CSV 名

#### Scenario: 显式 data_root 覆盖默认值
- **WHEN** 用户选择 Scenario 32 并设置 `data.dataset.data_root: /tmp/custom_scene32`
- **THEN** 系统 MUST 使用 `/tmp/custom_scene32` 构建 dataset
- **AND** 系统 MUST 仍在 metadata 中记录规范场景为 Scenario 32
