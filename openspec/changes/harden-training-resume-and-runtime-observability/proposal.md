...
- **BREAKING**：current checkpoint schema 不再把 validation loss 复制为 `test_loss`；不符合 current schema 的 checkpoint 直接拒绝，不保留迁移路径。
...
- `original-code-compatibility`：收紧 resume 路径、角色 schema、不可变 fingerprint 与 current checkpoint 恢复契约。
...
- checkpoint 与 sidecar 的新 schema 需要 focused rejection/compatibility tests；不符合 current schema 的历史 checkpoint 不再作为可恢复输入，新产物不保留含义错误的 `test_loss` alias。
