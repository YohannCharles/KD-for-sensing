...
from kd_sensing.data.deepsense6g_topk_candidate_manifest import build_topk_candidate_manifest


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Prepare DeepSense6G GPS Top8 candidate manifest.")
    parser.add_argument("--config", "-c", default="configs/deepsense6g_top8_selector.yaml")
...
def run_main(argv: list[str] | None = None) -> dict[str, Any]:
    ...
    return build_topk_candidate_manifest(
        cfg,
...
def _load_config(path: str | Path, overrides: list[str]) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"Top8 selector config must be a mapping: {path}")
    if overrides:
