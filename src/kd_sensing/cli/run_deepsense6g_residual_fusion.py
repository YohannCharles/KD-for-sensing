...
from kd_sensing.engine.deepsense6g_residual_fusion import run_deepsense6g_residual_fusion


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run DeepSense6G GPS-anchored residual fusion.")
    parser.add_argument("--config", "-c", default="configs/deepsense6g_residual_fusion.yaml")
...
def run_main(argv: list[str] | None = None) -> dict[str, Any]:
    ...
    return run_deepsense6g_residual_fusion(
        cfg,
...
def _load_config(path: str | Path, overrides: list[str]) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"Residual fusion config must be a mapping: {path}")
    if overrides:
