...
from kd_sensing.engine.mmw_town_gps_lidar_bgam import evaluate_mmw_town_gps_lidar_bgam


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate MMW Town GPS pseudo-history + LiDAR BGAM reranker.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = evaluate_mmw_town_gps_lidar_bgam(
        cfg,
        ckpt=args.ckpt,
...
def _load_config(path: str | Path, overrides: list[str]) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"MMW Town BGAM evaluation config must be a mapping: {path}")
