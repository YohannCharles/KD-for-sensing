...
class FusionTeacherModalityNet(nn.Module):
    def __init__(
        ...
        csi_hardening: dict | bool | None = None,
        ...
        if "csi" in self.modalities:
            self.csi_feature_extractor = PilotDualViewCSIEncoder(
                ...
                csi_hardening=csi_hardening,
                delay_taps=delay_taps,
                view_fusion=view_fusion,
                view_gate_warmup_epochs=view_gate_warmup_epochs,
                delay_view_warmup_epochs=delay_view_warmup_epochs,
                use_internal_gru=use_internal_gru,
                tokenizer=tokenizer,
            )
