...
def format_results_markdown(results: list[dict]) -> str:
    ...
    for row in results:
        lines.append("| " + " | ".join(_format_result_cell(row.get(column, "")) for column in columns) + " |")
    return "\n".join(lines) + "\n"
...
def _format_result_cell(value: Any) -> str:
...
def _beam_classification_metrics(logits: torch.Tensor, target: torch.Tensor, cfg: dict[str, Any]) -> dict[str, float]:
    ...
    return {
        ...
        "within_1": float(summary.get("within_1", math.nan)),
