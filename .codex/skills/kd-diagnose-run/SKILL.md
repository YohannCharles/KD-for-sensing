...
description: 只读诊断 KD-for-sensing 的本地训练、评估、topology-probing 产物、配置、协议 provenance 和仓库表面。用于排查运行失败、检查实验健康度、核对 ignored 产物或定位当前 CLI/config 漂移；除非用户另有明确授权，不修改本地产物或受保护文件。
---
...
2. 四模态主线使用 `tools/run_topology_predictor.py` 与 `tools/eval_topology_predictor.py`；先运行对应 `--help`，再选择 resolve/preflight/train、matrix 或 probing 动作。不要注册新的 console script。
...
```bash
conda run -n kd_mm_beam python tools/run_topology_predictor.py --help
conda run -n kd_mm_beam python tools/eval_topology_predictor.py --help
conda run -n kd_mm_beam pytest tests/test_four_modal_topology_predictor.py tests/test_beam_probe_diagnostic.py -q
make verify-quick
```
