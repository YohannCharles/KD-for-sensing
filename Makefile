...
RUFF ?= conda run -n kd_mm_beam ruff
OPENSPEC ?= openspec

.PHONY: verify verify-quick verify-cli-config verify-docs verify-compile verify-lint verify-full
...
verify-full: verify-compile verify-lint
	$(OPENSPEC) validate --all --strict
	$(PYTEST) -q
