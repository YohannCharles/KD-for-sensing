...
def test_raymobtime_analysis_accepts_single_task_objectives(tmp_path: Path):
    run_dirs = []
    for objective in ("current_beam_selection", "current_los_classification", "current_link_quality"):
        run_dir = tmp_path / objective
        run_dir.mkdir()
            ...
            json.dumps(
                {
                    ...
                    "val_los_f1": 0.2,
                    "val_link_mae": 1.5,
                    "enabled_modalities": ["coord"],
                    "objective": {"name": objective},
                    "prediction_setup": {"task_semantics": "current_snapshot_beam_selection"},
                }
            ),
            encoding="utf-8",
        )
        (run_dir / "train_log.json").write_text(json.dumps({"epoch_logs": []}), encoding="utf-8")
    ...
    result = analyze_raymobtime_modality_imbalance(exp_dirs=run_dirs, output_dir=str(tmp_path / "analysis"))

    assert sorted(run["objective"] for run in result["runs"]) == [
...
def test_raymobtime_analysis_uses_paired_best_top1_and_drop_contribution(tmp_path: Path):
    experiments = tmp_path / "experiments"
    evaluations = tmp_path / "evaluations"
    single = experiments / "s008_cil_beam_seed42"
    multi = experiments / "s008_multitask_cil_original_seed42"
    single.mkdir(parents=True)
    multi.mkdir(parents=True)
    (multi / "checkpoints").mkdir()
    (multi / "checkpoints" / "best.pth").write_bytes(b"")
    (multi / "checkpoints" / "best_top1.pth").write_bytes(b"")

    common_setup = {"task_semantics": "current_snapshot_beam_selection"}
        ...
        json.dumps(
            {
                ...
                "val_beam_top3": 0.7,
                "val_beam_dba": 0.6,
                "enabled_modalities": ["coord", "image", "lidar"],
                "objective": {"name": "current_beam_selection"},
                "prediction_setup": common_setup,
                "los_buckets": {
                    "LOS=0": {"val_beam_top1": 0.4, "val_beam_top3": 0.6, "beam_dba_current": 0.5, "sample_count": 2},
                    "LOS=1": {"val_beam_top1": 0.6, "val_beam_top3": 0.8, "beam_dba_current": 0.7, "sample_count": 3},
                },
            }
        ),
        encoding="utf-8",
    )
    (single / "train_log.json").write_text(json.dumps({"epoch_logs": []}), encoding="utf-8")
    (single / "final_config.yaml").write_text(
        "\n".join(
            [
                "data:",
                "  dataset:",
    ...
    multi_metrics = {
        ...
        "val_los_f1": 0.9,
        "val_link_mae": 2.0,
        "val_selection_multitask_loss": 3.0,
        "enabled_modalities": ["coord", "image", "lidar"],
        "objective": {"name": "selection_multitask"},
        "prediction_setup": common_setup,
        "modality_subsets": {
            "all": {"val_beam_top1": 0.3, "val_beam_dba": 0.45, "val_los_f1": 0.9, "val_link_mae": 2.0},
            "drop_coord": {"val_beam_top1": 0.25, "val_beam_dba": 0.40, "val_los_f1": 0.85, "val_link_mae": 2.1},
            "drop_image": {"val_beam_top1": 0.31, "val_beam_dba": 0.46, "val_los_f1": 0.90, "val_link_mae": 2.0},
            "drop_lidar": {"val_beam_top1": 0.10, "val_beam_dba": 0.20, "val_los_f1": 0.50, "val_link_mae": 5.0},
        },
    }
    (multi / "metrics.json").write_text(json.dumps(multi_metrics), encoding="utf-8")
    (multi / "train_log.json").write_text(
        json.dumps(
            {
                "epoch_logs": [
                    {
                    ...
                    {
                        "epoch": 2,
                        "val_selection_multitask_loss": 2.8,
                        "val_beam_top1": 0.35,
                        "val_los_f1": 0.91,
                        "val_link_mae": 1.9,
                    },
    ...
    (multi / "final_config.yaml").write_text(
        "\n".join(
            [
                "data:",
                "  dataset:",
    ...
    best_top1 = evaluations / "s008_multitask_cil_original_seed42_best_top1"
    best_top1.mkdir(parents=True)
    (best_top1 / "metrics.json").write_text(
        json.dumps(
            {
                **multi_metrics,
                "val_beam_top1": 0.52,
                "los_buckets": {
                    "LOS=0": {"val_beam_top1": 0.42, "val_beam_top3": 0.7, "beam_dba_current": 0.55, "sample_count": 2},
                    "LOS=1": {"val_beam_top1": 0.62, "val_beam_top3": 0.9, "beam_dba_current": 0.75, "sample_count": 3},
                },
            }
        ),
        encoding="utf-8",
    ...
    result = analyze_raymobtime_modality_imbalance(
        exp_dirs=[str(single), str(multi)],
        output_dir=str(tmp_path / "analysis"),
    )

    assert result["s009_external_validation"]["decision"] == "not_started"
    assert "second-stage cross-scene validation" in Path(result["outputs"]["report"]).read_text(encoding="utf-8")

    with Path(result["outputs"]["checkpoint_epoch_summary"]).open(encoding="utf-8", newline="") as f:
        checkpoint_rows = list(csv.DictReader(f))
    beam_row = next(row for row in checkpoint_rows if row["metric"] == "val_beam_top1")
    assert beam_row["reevaluation_status"] == "available"
    assert float(beam_row["reeval_val_beam_top1"]) == pytest.approx(0.52)

    with Path(result["outputs"]["beam_metrics_by_los_bucket"]).open(encoding="utf-8", newline="") as f:
        los_rows = list(csv.DictReader(f))
    assert any(row["checkpoint_view"] == "best_top1" and row["los_bucket"] == "LOS=1" for row in los_rows)

    with Path(result["outputs"]["grad_norms_by_task_modality"]).open(encoding="utf-8", newline="") as f:
        contribution_rows = list(csv.DictReader(f))
    lidar_beam = [
        row
        for row in contribution_rows
        if row["diagnostic_type"] == "contribution_from_modality_drop"
        and row["task"] == "beam_selection"
        and row["modality"] == "lidar"
        and row["metric"] == "beam_top1"
    ]
    assert lidar_beam
    assert float(lidar_beam[0]["contribution_score"]) == pytest.approx(0.2)
