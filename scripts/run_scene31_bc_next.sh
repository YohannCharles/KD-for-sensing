...
BASELINES=(amr_net_supervised amber_full_architecture)

usage() {
  cat <<'EOF'
Usage:
  bash scripts/run_scene31_bc_next.sh --group all_p0 --gpu 0

Options:
  --group NAME        b_p0, c_p0, bc_p0, all_p0, all, baselines.
  --root PATH         Output root. Default: outputs/scene31_bc_next.
  --manifest PATH     Scene31 next-round manifest CSV.
  --gpu ID            CUDA_VISIBLE_DEVICES value for train/eval commands.
...
case "$GROUP" in
  ...
  c_p0) RUNS=("${C_P0[@]}") ;;
  bc_p0) RUNS=("${BC_P0[@]}") ;;
  all_p0) RUNS=("${B_P0[@]}" "${C_P0[@]}" "${BC_P0[@]}" "${BASELINES[@]}") ;;
  all) RUNS=("${B_P0[@]}" "${C_P0[@]}" "${BC_P0[@]}" "${B_P1[@]}" "${C_P1[@]}" "${BASELINES[@]}") ;;
...
ensure_scene31_configs() {
  ...
  for run_name in "${RUNS[@]}"; do
    if [[ "$run_name" == "amr_net_supervised" || "$run_name" == "amber_full_architecture" ]]; then
      continue
    fi
    local cfg
...
config_for_run() {
  ...
  case "$run_name" in
    amr_net_supervised)
      echo "configs/fusion/amr_net_supervised.yaml"
      return 0
      ;;
    amber_full_architecture)
      echo "configs/fusion/amber_full_architecture.yaml"
      return 0
      ;;
  esac
  scene31_manifest_value "$MANIFEST" "$run_name" config_path
}
