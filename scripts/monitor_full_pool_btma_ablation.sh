...
while true; do
  {
    ...
    if [ -f "$output_root/pids.txt" ]; then
      while IFS=$'\t' read -r pid gpu uuid method; do
        ...
        [ ! -f "$output_root/$method/runtime_status.json" ] || tail -n 1 "$output_root/$method/runtime_status.json"
        latest="$(find "$output_root/assignments/$method" -name 'epoch_*.json' -type f -printf '%T@ %p\n' 2>/dev/null | sort -n | tail -n 1 | cut -d' ' -f2-)"
        [ -z "$latest" ] || tail -n 1 "$latest"
