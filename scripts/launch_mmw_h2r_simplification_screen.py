#!/usr/bin/env python3
"""Prepare or launch the fixed eight-GPU H2R simplification screen."""
...
PROTOCOL_ID = "mmw_h2r_simplification_screen_v1"
DEFAULT_OUTPUT = ROOT / "outputs/mmw_h2r_simplification_screen_v1"
...
SEED = 1
CANDIDATES = (
    ("Full-40", "full", "beam_power", 40, "full"),
    ("Full-10", "full", "beam_power", 10, "full"),
    ("Lite-10", "full", "label_topology", 10, "lite"),
    ("LiteMono-10", "full", "label_topology", 10, "mono"),
    ("Generic-10", "generic_confidence", "label_topology", 10, "lite"),
    ("GenericMono-10", "generic_confidence", "label_topology", 10, "mono"),
    ("Prototype-10", "prototype_topology", "label_topology", 10, "lite"),
    ("PrototypeMono-10", "prototype_topology", "label_topology", 10, "mono"),
)
...
def main() -> int:
    ...
    if gpus != tuple(range(8)):
        parser.error("The frozen H2R simplification screen requires --gpus 0,1,2,3,4,5,6,7.")
    if int(args.batch_size) != 64:
        parser.error("The frozen H2R simplification screen requires --batch-size 64.")
    if args.min_free_mib <= 0 or args.poll_seconds <= 0:
        parser.error("--min-free-mib and --poll-seconds must be positive.")
...
def prepare_plan(
    *,
    ...
    if not source_config.is_file() or not source_checkpoint.is_file():
        raise FileNotFoundError("H2R simplification source config/checkpoint is missing.")
    ...
    request = {
        ...
        "prototype_router_source_sha256": base._sha256(
            ROOT / "src/kd_sensing/models/prototype_health_router.py"
        ),
        "u_mask_model_source_sha256": base._sha256(ROOT / "src/kd_sensing/models/u_mask_beam_jepa.py"),
    ...
    if manifest_path.is_file():
        ...
        if existing.get("request_sha256") != request_sha256:
            raise ValueError(f"Existing H2R simplification plan differs from the frozen request: {manifest_path}")
    ...
    jobs = []
    for gpu, candidate in zip(gpus, CANDIDATES, strict=True):
...
def build_candidate_config(
    ...
    *,
    ...
    if candidate not in CANDIDATES:
        raise ValueError(f"Unknown H2R simplification candidate {candidate!r}.")
    name, profile, supervision, epochs, loss_profile = candidate
    template_name = "H2R-Power" if supervision == "beam_power" else "H2R-Label"
    ...
    config["model"]["primary"]["router_variant_config"]["evidence_profile"] = profile
    dynamic = config["loss"]["u_mask_beam_jepa"]["dynamic_router"]
    full = loss_profile == "full"
    monotonic_weight = 0.2 if loss_profile in {"full", "mono"} else 0.0
    dynamic.update(
        {
            "fused_decision_objective": "joint_hard_ce",
            "fused_decision_margin": 0.5,
            "quality_regression_weight": 0.2 if full else 0.0,
            "fused_utility_weight": 1.0,
            "frame_rank_weight": 0.2 if full else 0.0,
            "residual_anchor_weight": 0.01 if full else 0.0,
        }
    )
    dynamic["paired_joint"]["monotonic_weight"] = monotonic_weight
    config.pop("mmw_dynamic_router_screen", None)
    config["mmw_h2r_simplification_screen"] = {
        "protocol": PROTOCOL_ID,
        "candidate": name,
        "router_variant": "h2r",
        "evidence_profile": profile,
        ...
        "loss_profile": loss_profile,
        "fused_decision_objective": "joint_hard_ce",
        "quality_regression_weight": dynamic["quality_regression_weight"],
        "fused_decision_weight": dynamic["fused_utility_weight"],
        "monotonic_weight": monotonic_weight,
        "frame_rank_weight": dynamic["frame_rank_weight"],
        "residual_anchor_weight": dynamic["residual_anchor_weight"],
        "calibration_epochs": int(epochs),
        "seed": SEED,
