...
def parse_args():
    ...
    parser.add_argument('--model_arch', type=str, default='BothTeacher', choices=['BothTeacher', 'BothStudent'], help='Model architecture')
    parser.add_argument('--kd_mode', type=int, default=0, choices=[0, 1, 2], help='KD mode for student model; 0: no KD, 1: KD, 2: RKD')
...
def main():
    ...
    if args.model_arch == 'BothTeacher':
        # args.model_name = 'ImageTeacher_best.pth'
        args.model_name = 'ImageTeacher_noKD.pth'
        model = ImageModalityNet(args.feature_size, args.num_classes, GRU_PARAMS_teacher)
    elif args.model_arch == 'BothStudent':

        model = ImageStudentModalityNet(args.feature_size, args.num_classes, GRU_PARAMS_student)

        if args.kd_mode == 0:
            args.model_name = 'ImageStd_noKD.pth'
        elif args.kd_mode == 1:
            args.model_name = 'ImageStd_KD.pth'
        elif args.kd_mode == 2:
            args.model_name = 'ImageStd_RKD.pth'
        else:
